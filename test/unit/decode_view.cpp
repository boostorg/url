...
struct decode_view_test
{
    ...
    string_view plus_dec_str = "a uri test";
    const std::size_t dn = 10;
    decode_opts plus_opt;

    decode_view_test()
    {
        plus_opt.plus_to_space = true;
    }
    ...
    testDecodedView()
    {
        ...
        {
            std::string_view std_str = str;
            decode_view s(std_str, plus_opt);
            BOOST_TEST_EQ(s, plus_dec_str);
            BOOST_TEST_EQ(s.size(), dn);
        }
#endif
    ...
    testCompare()
    {
        ...
        // operators
        {
            ...
            // string literals
            {
                ...
                BOOST_TEST_NE(s, "a tri+test");
                BOOST_TEST_NE(s, "a vri+test");
                BOOST_TEST_NE(s, "a uri test");
                BOOST_TEST_LT(s, "a vri+test");
                BOOST_TEST_LT(s, "a vri test");
                BOOST_TEST_LE(s, "a vri+test");
                BOOST_TEST_LE(s, "a uri+test");
                BOOST_TEST_GT(s, "a tri+test");
                BOOST_TEST_NOT(s > "a uri+test");
                BOOST_TEST_GE(s, "a tri+test");
                BOOST_TEST_GE(s, "a uri+test");
            }
